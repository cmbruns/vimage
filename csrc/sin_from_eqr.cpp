...
// TODO: equirectangular blur function
...
void sin_from_equi(const PIXEL_TYPE* src, PIXEL_TYPE* dst, size_t nRows, size_t nCols, size_t nChans)
{
	...
	for(size_t row = 0; row < nRows; row++) 
    {
        ...
        double latitude = -std::numbers::pi * ((0.5 + row) / nRows - 0.5);  // range (-PI/2, +PI/2)
        // clamp to +- (pi - epsilon)
        ...
        double c = (nCols - 1)/2.0;  // center of scan line in pixel indices
        ...
        for (auto dpx = min_dst_px; dpx <= max_dst_px; ++dpx) {
            ...
            src1 = std::max(src1, src0);
            src1 = std::min(src1, int(nCols - 1));  // TODO: do we need to wrap here?
            std::ranges::fill(pixelSum, 0);

            for (auto spx = src0; spx <= src1; ++spx) {
                for (auto chan = 0; chan < nChans; ++chan) {
                    pixelSum[chan] += sp[nChans * spx + chan];
                }
            }
        ...
        // write wrapped pixels at boundaries
        auto wrap_window = std::min(min_dst_px - 1, max_dst_px - min_dst_px + 1);
        for(auto dp = 0; dp < wrap_window; ++dp) {
            for (auto chan = 0; chan < nChans; ++chan) {
                scan_buffer[nChans * (min_dst_px - dp - 1) + chan] = scan_buffer[nChans * (max_dst_px - dp) + chan];
                scan_buffer[nChans * (max_dst_px + dp + 1) + chan] = scan_buffer[nChans * (min_dst_px + dp) + chan];
            }
        }
